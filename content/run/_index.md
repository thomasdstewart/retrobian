...
Alternatively it is possible to run an existing virtual machine that I have created, either by streaming the disk images via http and using QEMU copy on write functionality, or downloading the images directly. These disk images are not stored in gitlab but on a machine of mine that may or may not disappear. However by following the instructions it should be straight forward to reproduce them. Note: it has disappeared and I've not replaced it with anything yet.
...
Given that DHCP clients are a new invention, all the machines have a statically configured IP of 192.168.122.10/24. If you install libvirt then usually network with a virbr0 bridge is created with this subnet. So you can either use these defaults or configure your own networking.
...
7. Once X has started you should be able to login with username "thomas" and password "Password99" alternativly you can telnet to the machine with:
